...
class CalculateHandicap(AbstractTask):
    ...
    def calculate_differential(
        gross_score: int, course_rating: Decimal, slope: int
    ) -> Decimal:
    ...
    def determine_lowest_differential(
        self, differentials: List[Decimal]
    ) -> List[Decimal]:
    ...
    def calculate_handicap_index(self, differentials) -> Decimal:
        ...
        handicap_index = (
            sum(lowest_differentials) / len(lowest_differentials)
        ) * Decimal("0.96")
    ...
    async def process_record(self):
        if self.event_name in ["INSERT", "DELETE"]:
            async with FootwedgeApiClient() as api_client:
                ordered_golf_rounds = await api_client.get_golf_rounds(
                    user_id=self.user_id
                )
                if not ordered_golf_rounds:
                    logger.warning(
                        f"The user_id: {self.user_id} does not have any golf_round records"
                    )
                    return
                ...
                success_message = f"Successfully calculated and added a new handicap: {handicap_index} for user: {self.user_id}"
