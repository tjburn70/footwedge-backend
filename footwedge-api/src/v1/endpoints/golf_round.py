...
def add_golf_rounds(
    golf_round_body: GolfRoundBody, user: CognitoUser = Depends(get_current_user)
):
    service = GolfRoundService(user=user, repo=golf_round_repo)
...
@router.get(
    "/{golf_round_id}/aggregate-stats", response_model=GetGolfRoundAggregateStats
)
...
def get_golf_rounds_by_user_id(
    user_id: str, _: str = Security(authorize_client, scopes=[READ_SCOPE])
):
